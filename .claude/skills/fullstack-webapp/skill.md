...
### Phase 0: 아이디어 명확성 판단 (오케스트레이터 직접 수행)

**구현 파이프라인 진입 전에** 사용자 입력의 구체성을 판단한다.

#### 판단 기준

| 신호 | 대응 |
|------|------|
| `idea.md`가 프로젝트 루트에 존재 | → `idea_miner` 스킬 실행 |
| 입력이 한 문장이거나 핵심 의사결정 포인트가 비어 있음 | → `idea_miner` 스킬 실행 |
| `idea_inquiry.md`가 LGTM 상태 + `spec/` 미존재 | → `spec_check` 실행 여부 사용자에게 제안 |
| `spec/`가 존재하거나 입력이 충분히 구체적 | → Phase 1로 바로 진행 |
| 사용자가 "바로 만들어줘" 등 Phase 0 스킵 의사 표명 | → Phase 1로 바로 진행 |
...
입력 수신
  ↓
idea.md 존재? OR 입력이 모호?
  ├── YES → [idea_miner 스킬 실행]
  │         ↓
  │         LGTM?
  │           ├── YES → spec_check 실행 여부 제안
  │           │         ↓
  │           │         spec/ 생성 완료 OR 스킵
  │           │         ↓ Phase 1
  │           └── NO  → 사용자 답변 대기 → 반복
  ...
  └── NO (입력 충분)
        ↓
        spec/ 존재? → YES → architect에게 spec/ 참고 지시
                    → NO  → 바로 Phase 1
```

#### spec/ 존재 시 architect 지시사항

`spec/`가 존재하면 architect에게 다음을 명시적으로 전달한다:

```
spec/ 폴더가 존재합니다. _workspace/ 작성 전에 반드시 읽어주세요.
- spec/01_prd.md → _workspace/00_input.md + 01_architecture.md 입력
- spec/02_architecture_preview.md → 01_architecture.md 초안
- spec/03_api_preview.md → 02_api_spec.md 초안
- spec/04_db_preview.md → 03_db_schema.md 초안
spec/의 가정 사항을 검증하고, 확정된 내용은 _workspace/로 상세화하여 이관하세요.
먼저 spec/ 문서가 올바르고 빈틈없는지 자체 검토하세요.
- 충분하면 그대로 _workspace/ 작성으로 진행
- 부족하거나 상충되면 spec/을 먼저 재작성/보강
- spec/을 재작성한 경우 이번 호출에서는 구현 단계로 넘어가지 말고 사용자에게 검토 후 `/fullstack-webapp` 재호출을 요청
```
...
3. 입력을 정리하여 `_workspace/00_input.md`에 저장한다 (`idea_inquiry.md`, `spec/`가 있으면 함께 통합)
...
단, `spec/`가 있는 상태에서 architect가 spec/ 재작성/보강을 수행한 경우에는 **이번 호출에서 Phase 2로 진입하지 않는다**.  
사용자가 보강된 spec/을 검토한 뒤 `/fullstack-webapp`을 다시 호출했을 때만 팀 실행을 시작한다.
...
| 에러 유형 | 전략 |
|----------|------|
| 요구사항 모호 (Phase 0) | `idea_miner` 스킬로 우회, 사용자 답변 후 재진입 |
| spec/ 품질 미흡 | architect가 spec/ 재작성 → 이번 호출 중단 → 사용자 검토 후 `/fullstack-webapp` 재호출 |
| 요구사항 모호 (Phase 1 진입 후) | 가장 일반적인 CRUD 패턴 적용, 가정 사항 문서화 |
